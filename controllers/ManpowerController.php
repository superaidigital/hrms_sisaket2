...
class ManpowerController {
    ...
    // 🌟 ฟังก์ชันใหม่: รับข้อมูลจาก Popup แล้วบันทึกลงฐานข้อมูล ส่งกลับเป็น JSON
    public function storeAjax() {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $data = [
                ...
                'position_name' => trim($_POST['position_name'] ?? ''),
                'level' => trim($_POST['level'] ?? ''),
                'status' => 'vacant', // บังคับให้เป็นว่างไว้ก่อน เพราะเดี๋ยวคนนี้จะเซฟทับเป็นคนครองเองตอนกดบันทึกพนักงาน
